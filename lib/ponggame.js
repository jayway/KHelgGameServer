...
function Game(io,player1,player2) {

  this.started = true;

  this.player1 = player1; 
  this.player2 = player2; 
  this.player1.addToGame(this,175,580,50,10);
  this.player2.addToGame(this,175,10,50,10);

  this.gameplanHeight = 600;
  this.gameplanWidth = 400; 
...
Game.prototype.checkWinning = function() {
  ...
  if(this.player1Points>=this.winningPoints) {
    this.player1.socket.emit("winning", {winner:this.player1.name, player1Points:this.player1Points, player2Points:this.player2Points});
    this.player2.socket.emit("winning", {winner:this.player1.name, player1Points:this.player1Points, player2Points:this.player2Points});
    console.log("Player 1 ("+this.player1.name+") has won!");
    this.started = false;
    winning = true;
  }
  if(this.player2Points>=this.winningPoints) {
    this.player1.socket.emit("winning", {winner:this.player2.name, player1Points:this.player1Points, player2Points:this.player2Points});
    this.player2.socket.emit("winning", {winner:this.player2.name, player1Points:this.player1Points, player2Points:this.player2Points});
  ...
  if(winning) {
    this.started = false;
    this.player1.state = "spectator";
    this.player2.state = "spectator";
    this.player1Points = 0;
    this.player2Points = 0;
  }
...
Game.prototype.broadcast = function() {
  ...
  var gameState = {
    ...
    players: {
      player1: {name: this.player1.name,score:this.player1Points},
      player2: {name: this.player2.name,score:this.player2Points}
    },
    bounds: {
  ...
  this.player1.socket.emit('step', gameState);
  this.player2.socket.emit('step', gameState);
};
